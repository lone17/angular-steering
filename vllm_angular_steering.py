...
Requirements:
    ...
    - enable_prefix_caching=False (REQUIRED when sweeping multiple angles)
...
def _detect_prefill_decode_phase(
    ...
    Uses max_query_len from the forward context:
    - Prefill: max_query_len > 1 (processing multiple input tokens)
    - Decode: max_query_len == 1 (generating one token at a time)

    Known limitation: when prefix caching is enabled and all but 1 prompt
    token is served from cache, vLLM recomputes only that 1 tail token in a
    step where max_query_len==1 — identical to a decode step. There is no
    metadata field in the flash_attn backend's forward context that
    distinguishes this from true decode (num_prefill_tokens is not exposed).
    As a result, prompt_only steering silently skips the tail token,
    producing outputs that differ from the no-cache case.

    The correct fix is to initialize LLM with enable_prefix_caching=False
    when sweeping multiple steering angles over the same prompts.
    ...
    try:
        ...
        if attn_meta is not None:
            # Check max_query_len — most authoritative available indicator.
            # Note: num_prefill_tokens would be more precise but is not
            # exposed in the flash_attn backend's forward context metadata.
            max_query_len = getattr(attn_meta, "max_query_len", None)
            if max_query_len is not None:
                return max_query_len == 1  # 1 = decode, >1 = prefill

            # Fallback: num_decode_tokens
            if hasattr(attn_meta, "num_decode_tokens"):
                return attn_meta.num_decode_tokens > 0

            # Fallback: num_prefill_tokens
...
class AngularSteering:
    ...
    Requires vLLM to be initialized with enforce_eager=True and
    enable_prefix_caching=False for hooks to work correctly.
    """
    ...
    def __init__(self, llm):
        ...
        Args:
            llm: vLLM LLM instance (must have enforce_eager=True and
                 enable_prefix_caching=False)
        """
        self.llm = llm
